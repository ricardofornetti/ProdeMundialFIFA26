    ...
    <div className="max-w-[340px] xs:max-w-md w-full mx-auto bg-white p-6 sm:p-10 rounded-[2rem] sm:rounded-[3rem] shadow-2xl border border-slate-100 animate-fade-in relative overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="block text-[11px] font-black text-slate-500 mb-1 uppercase tracking-widest">{mode === 'login' ? 'Usuario o Email' : 'Correo Electrónico'}</label>
            <input type={mode === 'login' ? "text" : "email"} value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-2.5 sm:px-5 sm:py-3 rounded-xl border-2 border-slate-50 focus:border-indigo-600 outline-none transition-all font-bold text-slate-800 bg-slate-50/50 text-sm sm:text-base" placeholder={mode === 'login' ? "Tu usuario o email" : "ejemplo@prode.com"} required />
          </div>

          <div>
            <label className="block text-[11px] font-black text-slate-500 mb-1 uppercase tracking-widest">Contraseña</label>
            <div className="relative">
              <input 
                type={showPassword ? "text" : "password"} 
                value={password} 
                onChange={(e) => setPassword(e.target.value)} 
                className="w-full px-4 py-2.5 sm:px-5 sm:py-3 rounded-xl border-2 border-slate-50 focus:border-indigo-600 outline-none transition-all font-bold text-slate-800 bg-slate-50/50 text-sm sm:text-base" 
                placeholder="••••••••" 
                required 
              />
              <button 
            ...
            {mode === 'login' && (
              <button type="button" onClick={() => setMode('forgot-password')} className="mt-2 block text-[10px] font-black text-indigo-600 uppercase tracking-widest hover:underline">¿Olvidaste tu contraseña?</button>
            )}
          </div>
        ...
        <div className="space-y-2 sm:space-y-3 pt-2">
          <button type="submit" disabled={isLoading} className="w-full py-4 sm:py-5 bg-indigo-600 text-white font-black rounded-2xl shadow-xl hover:bg-indigo-700 transition-all uppercase tracking-widest text-sm sm:text-base disabled:opacity-50">
          ...
          {mode === 'login' && (
            <button 
              type="button" 
              onClick={() => setMode('register')}
              className="w-full py-4 sm:py-5 bg-indigo-600 text-white font-black rounded-2xl shadow-xl hover:bg-indigo-700 transition-all uppercase tracking-widest text-sm sm:text-base"
            >
              REGISTRARSE
            </button>
          )}
