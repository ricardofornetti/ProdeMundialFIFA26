        ...
        {groups.map((group, idx) => {
          ...
          return (
            <div key={idx} className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden">
              ...
              {/* TABLA CON SCROLL HORIZONTAL COMPLETA Y COLUMNA IZQUIERDA STICKY */}
                ...
                <table className="w-full text-left border-collapse min-w-[500px] sm:min-w-full">
                  <thead>
                    <tr className="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-700">
                      {/* Columna de equipo Sticky para conservar legibilidad al scrollear hacia el costado */}
                      <th className="sticky left-0 bg-[#f8fafc] dark:bg-[#1e293b] z-20 px-4 py-3.5 text-[10px] font-black text-slate-400 tracking-widest min-w-[145px] sm:min-w-[210px] shadow-[3px_0_6px_-2px_rgba(0,0,0,0.08)] dark:shadow-[3px_0_6px_-1px_rgba(0,0,0,0.3)]">
                        Equipo
                      </th>
                      <th className="px-2 py-3.5 text-[10px] font-black text-slate-400 uppercase text-center w-11 sm:w-16">PJ</th>
                      <th className="px-2 py-3.5 text-[10px] font-black text-slate-400 uppercase text-center w-11 sm:w-12">G</th>
                      <th className="px-2 py-3.5 text-[10px] font-black text-slate-400 uppercase text-center w-11 sm:w-12">E</th>
                      <th className="px-2 py-3.5 text-[10px] font-black text-slate-400 uppercase text-center w-11 sm:w-12">P</th>
                      <th className="px-2 py-3.5 text-[10px] font-black text-slate-400 uppercase text-center w-11 sm:w-12">GF</th>
                      <th className="px-2 py-3.5 text-[10px] font-black text-slate-400 uppercase text-center w-11 sm:w-12">GC</th>
                      <th className="px-2 py-3.5 text-[10px] font-black text-slate-400 uppercase text-center w-12 sm:w-16">DG</th>
                      <th className="px-4 py-3.5 text-[10px] font-black text-slate-900 dark:text-white uppercase text-center bg-slate-100 dark:bg-slate-700 w-16 sm:w-20">Pts</th>
                    </tr>
                  </thead>
                    ...
                    {stats.map((team, tIdx) => {
                      ...
                      return (
                        <tr key={tIdx} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors h-14">
                          {/* Celda del equipo Sticky (mantiene nombre legible completa y previene wrap feo) */}
                          <td className="sticky left-0 bg-white dark:bg-slate-800 z-10 px-4 py-3 shadow-[3px_0_6px_-2px_rgba(0,0,0,0.08)] dark:shadow-[3px_0_6px_-1px_rgba(0,0,0,0.3)]">
                            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                              ...
                              <span className="text-[10px] sm:text-xs font-bold uppercase truncate whitespace-nowrap min-w-0 flex-1 text-slate-800 dark:text-slate-100">
                                {team.name}
                              </span>
                            </div>
                          </td>
                          
                          <td className="px-2 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 text-center tabular-nums">{team.pj}</td>
                          <td className="px-2 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 text-center tabular-nums">{team.g}</td>
                          <td className="px-2 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 text-center tabular-nums">{team.e}</td>
                          <td className="px-2 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 text-center tabular-nums">{team.p}</td>
                          <td className="px-2 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 text-center tabular-nums">{team.gf}</td>
                          <td className="px-2 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 text-center tabular-nums">{team.gc}</td>
                          <td className={`px-2 py-3 text-xs font-bold text-center tabular-nums ${team.dg > 0 ? 'text-green-600' : team.dg < 0 ? 'text-red-600' : 'text-slate-500'}`}>
                            {team.dg > 0 ? `+${team.dg}` : team.dg}
                          </td>
                          <td className="px-4 py-3 text-xs sm:text-sm font-black text-slate-900 dark:text-white text-center bg-slate-50/50 dark:bg-slate-700/50 tabular-nums">{team.pts}</td>
                        </tr>
                      );
                    })}
                  </tbody>
