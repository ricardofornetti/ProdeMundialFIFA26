        ...
        {groups.map((group, idx) => {
          ...
          return (
            <div key={idx} className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden">
              ...
              <div className="sm:hidden overflow-x-auto -webkit-overflow-scrolling-touch scrollbar-none">
                <div className="divide-y divide-slate-100 dark:divide-slate-700/50 min-w-[340px]">
                  {/* Header perfectamente alineado */}
                  <div className="flex items-center px-2.5 py-3 bg-slate-50/50 dark:bg-slate-800/30 text-[10px] font-black text-slate-400 uppercase tracking-wider">
                    <div className="flex-1 min-w-0 pr-1.5">Equipo</div>
                    <div className="w-8 text-center shrink-0">PJ</div>
                    <div className="w-10 text-center shrink-0">DG</div>
                    <div className="w-11 text-center shrink-0">Pts</div>
                    ...
                    return (
                      <div key={tIdx} className="flex flex-col">
                        <button 
                          ...
                          className="w-full flex items-center px-2.5 py-3 min-h-[56px] text-left hover:bg-slate-50 dark:hover:bg-slate-700/10 transition-colors focus:outline-none"
                        >
                          {/* Posición, Bandera y Nombre */}
                          <div className="flex-1 flex items-center gap-1.5 min-w-0 pr-1.5 opacity-100">
                            <span className={`text-[10px] font-black w-3.5 shrink-0 text-center ${isClassified ? 'text-green-500' : 'text-slate-300'}`}>
                              {tIdx + 1}
                            </span>
                            <div className="w-5 h-3.5 overflow-hidden rounded shadow-xs border border-slate-100 dark:border-slate-700 shrink-0">
                              <img src={TEAM_FLAGS[team.flag] || TEAM_FLAGS['FIFA']} alt={team.name} className="w-full h-full object-cover" />
                            </div>
                            <span className={`text-[10px] xs:text-[11px] font-bold uppercase truncate whitespace-nowrap min-w-0 flex-1 ${team.flag === 'FIFA' ? 'text-slate-300 italic' : 'text-slate-800 dark:text-slate-100'}`}>
                              {team.name}
                            </span>
                          </div>

                          {/* PJ */}
                          <div className="w-8 text-center shrink-0 text-xs font-semibold text-slate-500 dark:text-slate-400 tabular-nums">
                            {team.pj}
                          </div>

                          {/* DG */}
                          <div className={`w-10 text-center shrink-0 text-xs font-semibold tabular-nums ${team.dg > 0 ? 'text-green-600' : team.dg < 0 ? 'text-red-600' : 'text-slate-500 dark:text-slate-400'}`}>
                            {team.dg > 0 ? `+${team.dg}` : team.dg}
                          </div>

                          {/* PTS (Destacada) */}
                          <div className="w-11 text-center shrink-0 text-xs font-black text-slate-900 dark:text-white tabular-nums bg-slate-100 dark:bg-slate-700 py-1 rounded-md">
                            {team.pts}
                          </div>

                          {/* Chevron */}
                          <div className="w-5 flex justify-end shrink-0 text-slate-400 dark:text-slate-50">
                            <svg 
